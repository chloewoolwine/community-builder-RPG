...
func update_specific_pixel(chunk_id: Vector2i, square_id:Vector2i) -> void:
	var chunk_data: ChunkData = chunk_atlas[chunk_id]
	var image_pos: Vector2i = chunk_id - top_left_chunk
	var img_x: int = (image_pos.x * chunk_data.chunk_size.x) + square_id.x
	var img_y: int = (image_pos.y * chunk_data.chunk_size.y) + square_id.y
	...
	till.material.set("shader_parameter/mod_color_tex", ImageTexture.create_from_image(moisture_gradient_map))
	till.material.set("shader_parameter/global_corner_pos", gradient_top_left_pos)
...
func till_square(overall_location: Vector2i) -> void: 
	till.set_cell(overall_location, 0, Vector2i.ZERO)
