...
## If true, uses exported atlas_id
@export var use_external_atlas_id: bool = false
## Keep track of the atlas ID
@export var external_atlas_id: int
...
func update_tileset() -> void:
	...
	if use_external_atlas_id: 
		_atlas_id = external_atlas_id
	else:
		_atlas_id = 0
...
func _update_tile_isometric(_world_cell: Vector2i) -> void:
	...
	var _top_left: Vector2i = _world_cell + NEIGHBOURS_ISOMETRIC[location.TOP_LEFT]
	var _low_left: Vector2i = _world_cell + NEIGHBOURS_ISOMETRIC[location.LOW_LEFT] + isometric_fix
	var _top_right: Vector2i = _world_cell + NEIGHBOURS_ISOMETRIC[location.TOP_RIGHT] + isometric_fix
	var _low_right: Vector2i = _world_cell + NEIGHBOURS_ISOMETRIC[location.LOW_RIGHT]
...
func _is_world_tile_sketched(_world_cell: Vector2i) -> bool:
	...
	if is_base_layer and _atlas_coords == full_tile:#base should always draw
		return true
	if _atlas_coords == full_tile && _source_id == external_atlas_id: 
		if debug:
